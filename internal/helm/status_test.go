...
func TestCheckReleaseHealth_Deployed(t *testing.T) {
	...
	healthy, msg, err := helm.CheckReleaseHealth(context.Background(), fakeHelmStatus(out, nil), "/etc/rancher/k3s/k3s.yaml", "appliance", "ace-system")
	if err != nil {
		t.Fatal(err)
	}
	if !healthy {
...
func TestCheckReleaseHealth_ReleaseMissing(t *testing.T) {
	healthy, msg, err := helm.CheckReleaseHealth(context.Background(), fakeHelmStatus("", errors.New("release: not found")), "/etc/rancher/k3s/k3s.yaml", "appliance", "ace-system")
	if err != nil {
		t.Fatal(err)
	}
	if healthy {
...
func TestCheckReleaseHealth_PropagatesHelmFailure(t *testing.T) {
	_, _, err := helm.CheckReleaseHealth(context.Background(), fakeHelmStatus("", errors.New("connection refused")), "/etc/rancher/k3s/k3s.yaml", "appliance", "ace-system")
