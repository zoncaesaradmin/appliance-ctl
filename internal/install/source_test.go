...
func TestOfflineSource_SelectsPrimaryChartAndOptionalWorkflowsArtifacts(t *testing.T) {
	...
	resolved, _, err := source.Resolve(context.Background(), "builder")
	if err != nil {
		t.Fatalf("expected bundle to resolve, got: %v", err)
	}
...
func TestOfflineSource_RejectsWorkflowsChartWithoutCRDs(t *testing.T) {
	...
	_, _, err = source.Resolve(context.Background(), "builder")
	if err == nil {
...
func TestOfflineSource_RequiresHostPackages(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "host-packages") {
		t.Fatalf("expected host-packages error, got: %v", err)
	}
}
